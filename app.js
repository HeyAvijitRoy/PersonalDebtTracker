...
// Optional: App Check (requires env.js with your site key)
import {
    initializeAppCheck,
    ReCaptchaV3Provider
...
if (!firebaseConfig || !firebaseConfig.apiKey) {
    console.error('Missing Firebase config. Create env.js from env.example.js');
    showModal?.('Config Error', 'Missing Firebase config. Create env.js from env.example.js');
    throw new Error('Missing Firebase config');
}
const appId = firebaseConfig.projectId;

if (location.hostname === 'localhost') {
    // Allows testing App Check locally without real captcha
...
function initFirebase() {
    try {
        ...
        // App Check (only if you set window.__RECAPTCHA_V3_SITE_KEY in env.js)
        if (window.__RECAPTCHA_V3_SITE_KEY) {
            initializeAppCheck(app, {
                provider: new ReCaptchaV3Provider(window.__RECAPTCHA_V3_SITE_KEY),
...
function setupFirestoreListener(uid) {
    ...
    unsubscribe = onSnapshot(cardsCollection, (snapshot) => {
        ...
        window.__latestCards = fetchedCards.map(x => ({
            ...x
        }));
...
function computeFicoHints(cards) {
    ...
    for (const c of cards) {
        ...
        if (util > 80) {
            over80++;
            sumTo80 += bal - (0.80 * lim);
        }
        if (util > 50) {
            over50++;
            sumTo50 += bal - (0.50 * lim);
        }
        if (util > 30) {
            over30++;
            sumTo30 += bal - (0.30 * lim);
        }
        ...
        if (next !== undefined) {
            ...
            if (dollarsToDrop > 0.01) nudges.push({
                name: c.name,
                currentUtil: util,
                nextThreshold: next,
                dollarsToDrop
    ...
    if (!btTargetSelect.value) {
        ...
        if (zeroApr.length === 1) {
            ...
            const byRoom = [...cards]
                .map(c => ({
                    ...c,
                    room: (+c.creditLimit || 0) - (+c.balance || 0)
                }))
...
cardList?.addEventListener('click', async (e) => {
    ...
    if (saveBtn) {
        ...
        try {
            ...
            if (idx !== -1) window.__latestCards[idx] = {
