...
if (!firebaseConfig || !firebaseConfig.apiKey) {
  console.error('[Config] Missing Firebase config. Ensure env.js loads BEFORE app.js.');
  authStatus && (authStatus.textContent = 'Config missing: ensure env.js loads before app.js.');
  // Add a visible banner so it’s obvious in UI
  const warn = document.createElement('div');
  warn.className = 'max-w-5xl mx-auto my-3 p-3 rounded border bg-amber-50 text-amber-800';
  warn.textContent = 'Missing Firebase config. Did you include env.js before app.js?';
...
function initFirebase() {
  try {
    ...
    getRedirectResult(auth).then((res) => {
      if (res?.user) console.log('[Auth] Redirect success for', res.user.email);
    }).catch((err) => {
      console.warn('[Auth] Redirect error:', err?.code, err?.message);
      if (authStatus && err?.code === 'auth/unauthorized-domain') {
        authStatus.textContent = 'Add localhost / 127.0.0.1 to Authorized domains in Firebase Auth.';
      }
    });
    ...
    authStatus && (authStatus.textContent = 'Failed to initialize Firebase. Check your configuration.');
  }
}
  ...
  try {
    ...
    try {
      ...
      if (err?.code === 'auth/unauthorized-domain') {
        showModal('Domain not authorized',
    ...
    authStatus && (authStatus.textContent = `${err?.code || 'auth/error'} — ${err?.message || 'Sign-in failed.'}`);
    showModal('Sign-in failed', `${err?.code || 'auth/error'} — ${err?.message || 'Try again.'}`);
  }
});
